...
pub fn free_variable(prog: &[knormal::Stat]) -> HashSet<String> {
  let mut free: HashSet<String> = HashSet::new();
  let mut rem: HashSet<String> = HashSet::new();
  for stat in prog {
    let (f, r) = stat.free_variable();
    // free += f - rem
    free = free.union(
...
fn test_free_variable() {
  assert_eq!(
    knormal::Stat::Assign("x".to_string(), knormal::Expr::Op2(crate::ast::Op2::Add, "y".to_string(), "z".to_string())).free_variable(),
